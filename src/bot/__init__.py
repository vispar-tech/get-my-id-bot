...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user or not chat:
        await update.message.reply_text("Unable to get user or chat information.")
        return
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text = (
        ...
        "User ID - your unique identifier in telegram, which you can use in your telegram bot. "
        "Read more: https://core.telegram.org/bots/api#user\n\n"
        "Commands:\n"
        "/start - Get your user ID and current chat ID\n"
        "/help - Show this help message\n\n"
        "You can also forward any message to me to get the sender's ID and original chat ID."
    )
    if not update.message:
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    response_parts = [f"Your user ID: {user_id}", f"Current chat ID: {chat_id}"]

    # Check if message is forwarded
    if message.forward_from:
        # Message forwarded from a user
        response_parts.append(f"Forwarded from user ID: {message.forward_from.id}")
        ...
        response_parts.append(f"Forwarded from chat ID: {message.forward_from_chat.id}")
        response_parts.append(
            f"Forwarded from chat title: {message.forward_from_chat.title}"
        )
        if message.forward_from_chat.username:
            response_parts.append(
                f"Forwarded from chat username: @{message.forward_from_chat.username}"
            )
    ...
    if message.reply_to_message:
        ...
        if reply_user:
            ...
            if reply_user.username:
                response_parts.append(f"Replying to username: @{reply_user.username}")
